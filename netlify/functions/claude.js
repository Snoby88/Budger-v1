...
import { TrendingUp, Target, Plus, Trash2, Wallet, ArrowUpCircle, ArrowDownCircle, Brain, Upload, Key, FileText } from "lucide-react";
...
export default function App() {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (file.type === "application/pdf") {
      try {
        ...
        setAiError("Kunne ikke læse PDF: " + err.message);
      }
    } else {
  ...
  const runAiAnalysis = async () => {
    ...
    setAiError("");
    setAiAnalysis(null);
    const txSummary = transactions.map(t => `${t.date} | ${t.type === "income" ? "Indtægt" : "Udgift"} | ${t.category} | ${t.description} | ${t.amount} kr`).join("\n");
  ...
  const importFromBank = () => {
    ...
    else alert("Ingen transaktioner fundet.");
  };
    ...
    <div style={s.root}>
      ...
      <div style={s.monthBar}>
        <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} style={s.select}>
          {getLast12Months().map(m => <option key={m} value={m}>{MONTHS[parseInt(m.slice(5)) - 1]} {m.slice(0, 4)}</option>)}
        </select>
      </div>
          ...
          <>
            ...
            <div style={s.card}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                <h3 style={{ ...s.cardTitle, margin: 0 }}>Upload kontoudtog (PDF eller CSV)</h3>
              </div>
              <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>Upload dit kontoudtog fra banken – både PDF og CSV understøttes!</p>
              <div style={{ border: "2px dashed #2d3f5e", borderRadius: 12, padding: 30, textAlign: "center" }}>
                ...
                <label htmlFor="fileInput" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, cursor: "pointer" }}>
                  ...
                  <span style={{ color: "#94a3b8", fontSize: 14 }}>{fileName ? `✅ ${fileName}` : "Klik for at vælge PDF eller CSV fil"}</span>
                </label>
              </div>
              {uploadedText && (
                <div style={{ marginTop: 12 }}>
                  <div style={{ background: "#0d1520", borderRadius: 8, padding: 12, fontSize: 11, color: "#64748b", maxHeight: 80, overflow: "hidden" }}>
                    {uploadedText.slice(0, 200)}...
                  </div>
                  <button onClick={importFromBank} style={{ ...s.confirmBtn, width: "100%", marginTop: 8, padding: "10px 0" }}>📥 Importér transaktioner fra fil</button>
                </div>
              )}
            </div>
            ...
            {aiAnalysis && (
              <>
                <div style={{ ...s.card, textAlign: "center", borderColor: "#D4A85344" }}>
                  ...
                  <div style={{ color: "#64748b", fontSize: 13, marginBottom: 12 }}>Din finansielle sundhedsscore</div>
                  <p style={{ color: "#e2e8f0", fontSize: 14, lineHeight: 1.6 }}>{aiAnalysis.overblik}</p>
                </div>
                <div style={s.chartsRow}>
...
const styles = {
  ...
  header: { display: "flex", alignItems: "center", gap: 20, padding: "16px 24px", borderBottom: "1px solid #1e2d45", background: "#0d1520", position: "sticky", top: 0, zIndex: 100 },
  ...
  kpiLabel: { display: "flex", alignItems: "center", gap: 6, color: "#64748b", fontSize: 12, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.5px" },
  ...
  cardTitle: { color: "#94a3b8", fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: "1px", margin: "0 0 16px 0" },
  ...
  empty: { color: "#334155", textAlign: "center", padding: "30px 0", fontSize: 14 },
  ...
  label: { display: "block", fontSize: 12, color: "#64748b", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" },
  input: { width: "100%", boxSizing: "border-box", background: "#1a2235", border: "1px solid #2d3f5e", color: "#e2e8f0", borderRadius: 8, padding: "9px 12px", fontSize: 14 },
