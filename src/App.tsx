...
export default function App() {
  ...
  const [apiKey, setApiKey] = useState(
    () => localStorage.getItem('budget_apikey') || ''
  );
  ...
  useEffect(() => {
    localStorage.setItem('budget_apikey', apiKey);
  }, [apiKey]);
  ...
  const runAiAnalysis = async () => {
    ...
    const txSummary = transactions
      .map(
        (t) =>
          `${t.date} | ${t.type === 'income' ? 'Indtægt' : 'Udgift'} | ${
            t.category
          } | ${t.description} | ${t.amount} kr`
      )
      .join('\n');
  ...
  const importFromBank = () => {
    ...
    if (imported.length > 0) {
      ...
      alert(`✅ ${imported.length} transaktioner importeret!`);
    } else
      alert('Ingen transaktioner fundet. Format: Dato, Beskrivelse, Beløb');
  };
  ...
  const PieTooltip = ({ active, payload }) =>
    active && payload?.length ? (
      ...
      >
        ...
        <p style={{ color: payload[0].payload.fill, fontSize: 13 }}>
  ...
  return (
    <div style={s.root}>
      <div style={s.header}>
        <div style={s.logo}>
          <Wallet size={22} color="#D4A853" />
          <span style={s.logoText}>BudgetPro</span>
        </div>
        <nav style={s.nav}>
        ...
        >
          {getLast12Months().map((m) => (
            <option key={m} value={m}>
              {MONTHS[parseInt(m.slice(5)) - 1]} {m.slice(0, 4)}
            </option>
          ))}
        </select>
      </div>
      ...
      <div style={s.content}>
        {activeTab === 'dashboard' && (
          <>
            <div style={s.kpiRow}>
              <div style={{ ...s.kpi, borderColor: '#D4A853' }}>
                <div style={s.kpiLabel}>
                  <ArrowUpCircle size={14} color="#D4A853" /> Indtægt
                </div>
                <div style={{ ...s.kpiValue, color: '#D4A853' }}>
                  {formatDKK(totalIncome)}
                </div>
              </div>
              <div style={{ ...s.kpi, borderColor: '#E8734A' }}>
                <div style={s.kpiLabel}>
                  <ArrowDownCircle size={14} color="#E8734A" /> Udgifter
                </div>
                <div style={{ ...s.kpiValue, color: '#E8734A' }}>
                  {formatDKK(totalExpense)}
                </div>
              </div>
              <div
                style={{
                  ...s.kpi,
                  borderColor: balance >= 0 ? '#34D399' : '#F87171',
                }}
              >
                <div style={s.kpiLabel}>
                  <Wallet
              ...
              <div style={{ ...s.kpi, borderColor: '#6B8CFF' }}>
                <div style={s.kpiLabel}>
                  <TrendingUp size={14} color="#6B8CFF" /> Opsparingsrate
                </div>
                <div style={{ ...s.kpiValue, color: '#6B8CFF' }}>
                  {savingsRate}%
                </div>
              </div>
            </div>
                ...
                .map((t) => (
                  <div key={t.id} style={s.txRow}>
                    <div
                      style={{
                        ...s.txDot,
                        background: t.type === 'income' ? '#34D399' : '#E8734A',
                      }}
                    />
                    <div style={{ flex: 1 }}>
                      <div style={s.txDesc}>{t.description || t.category}</div>
                      <div style={s.txMeta}>
                        {t.category} · {t.date}
                      </div>
                    </div>
                    <div
                      style={{
                        ...s.txAmt,
                        color: t.type === 'income' ? '#34D399' : '#E8734A',
                      }}
                    >
                      {t.type === 'income' ? '+' : '-'}
                      {formatDKK(t.amount)}
        ...
        {activeTab === 'transaktioner' && (
          <div style={s.card}>
            ...
            {monthTx.length === 0 && (
              <div style={s.empty}>Ingen transaktioner denne måned</div>
            )}
            {monthTx
        ...
        {activeTab === 'mål' && (
          <>
            ...
            >
              <button onClick={() => setShowGoalForm(true)} style={s.addBtn}>
                <Plus size={16} /> Nyt mål
              </button>
            </div>
            <div style={s.kpiRow}>
              <div style={{ ...s.kpi, borderColor: '#D4A853' }}>
                <div style={s.kpiLabel}>
                  <Target size={14} color="#D4A853" /> Aktive mål
                </div>
                <div style={{ ...s.kpiValue, color: '#D4A853' }}>
                  {goals.length}
                </div>
              </div>
              <div style={{ ...s.kpi, borderColor: '#4ECDC4' }}>
                <div style={s.kpiLabel}>
                  <TrendingUp size={14} color="#4ECDC4" /> Samlet opsparet
                </div>
                <div style={{ ...s.kpiValue, color: '#4ECDC4' }}>
                  {formatDKK(goals.reduce((s, g) => s + g.saved, 0))}
                </div>
              </div>
              <div style={{ ...s.kpi, borderColor: '#A78BFA' }}>
                <div style={s.kpiLabel}>
                  <Target size={14} color="#A78BFA" /> Samlet målsum
                </div>
                <div style={{ ...s.kpiValue, color: '#A78BFA' }}>
                  {formatDKK(goals.reduce((s, g) => s + g.target, 0))}
                </div>
              </div>
            </div>
            <div style={s.goalsGrid}>
              {goals.map((g) => {
                ...
                return (
                  <div
                    key={g.id}
                    style={{ ...s.goalCard, borderColor: g.color + '44' }}
                  >
                    <div style={s.goalHeader}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={s.goalMeta}>
                          {formatDKK(g.saved)} af {formatDKK(g.target)}
                        </div>
                      </div>
                      <button onClick={() => deleteGoal(g.id)} style={s.delBtn}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                    ...
                    <div style={s.goalStats}>
                      ...
                      <span style={{ color: '#64748b' }}>
                        Mangler {formatDKK(g.target - g.saved)}
                      </span>
                    </div>
                    <div style={s.goalActions}>
                      <input
                        ...
                        value={addGoalAmount[g.id] || ''}
                        onChange={(e) =>
                          setAddGoalAmount((prev) => ({
                            ...prev,
                            [g.id]: e.target.value,
                          }))
                        }
                      ...
                      <button
                        ...
                        style={{ ...s.goalBtn, background: g.color }}
                      >
                        <Plus size={14} />
                      </button>
                    </div>
          ...
          <>
            <div style={{ ...s.card, borderColor: '#D4A85344' }}>
              ...
              <p style={{ color: '#475569', fontSize: 11, marginTop: 8 }}>
                Opret gratis nøgle på console.anthropic.com
              </p>
            </div>
              ...
              >
                ...
                >
                  ...
                  <span style={{ color: '#94a3b8', fontSize: 14 }}>
                    {uploadedText ? '✅ Fil indlæst!' : 'Klik for at vælge fil'}
                  </span>
                </label>
              </div>
              {uploadedText && (
            ...
            >
              <Brain size={18} />
              {aiLoading ? 'Analyserer...' : '🤖 Analysér med AI'}
            </button>
            ...
            {aiAnalysis && (
              <>
                ...
                >
                  ...
                  <div
                    style={{ color: '#64748b', fontSize: 13, marginBottom: 12 }}
                  >
                    Din finansielle sundhedsscore
                  </div>
                  <p
                    style={{ color: '#e2e8f0', fontSize: 14, lineHeight: 1.6 }}
                  >
                    {aiAnalysis.overblik}
                  </p>
                </div>
                <div style={s.chartsRow}>
                  <div style={{ ...s.card, borderColor: '#34D39944' }}>
                    <h3 style={{ ...s.cardTitle, color: '#34D399' }}>
                      ✅ Styrker
                    </h3>
                    {aiAnalysis.styrker?.map((str, i) => (
                      <div
                        key={i}
                        style={{
                          display: 'flex',
                  ...
                  <div style={{ ...s.card, borderColor: '#E8734A44' }}>
                    <h3 style={{ ...s.cardTitle, color: '#E8734A' }}>
                      ⚠️ Opmærksomhed
                    </h3>
                    {aiAnalysis.advarsler?.map((a, i) => (
                      <div
                        key={i}
                        style={{
                          display: 'flex',
                ...
                <div style={s.card}>
                  <h3 style={{ ...s.cardTitle, color: '#6B8CFF' }}>
                    🚀 Vækststrategier
                  </h3>
                  <div style={s.goalsGrid}>
        ...
        <div style={s.overlay} onClick={() => setShowForm(false)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={s.typeToggle}>
              <button
                onClick={() =>
                  setForm((f) => ({ ...f, type: 'income', category: 'Løn' }))
                }
                style={{
                  ...s.typeBtn,
                  ...(form.type === 'income' ? s.typeBtnIncomeActive : {}),
                }}
              >
                <ArrowUpCircle size={15} /> Indtægt
              </button>
              <button
                onClick={() =>
                  setForm((f) => ({
                ...
                style={{
                  ...s.typeBtn,
                  ...(form.type === 'expense' ? s.typeBtnExpenseActive : {}),
                }}
              >
                <ArrowDownCircle size={15} /> Udgift
              </button>
            </div>
            <div style={s.field}>
              <label style={s.label}>Beløb (DKK)</label>
              <input
                type="number"
                placeholder="0"
                value={form.amount}
            ...
            <div style={s.modalBtns}>
              ...
              <button onClick={addTransaction} style={s.confirmBtn}>
                Gem transaktion
              </button>
            </div>
          </div>
      ...
      {showGoalForm && (
        <div style={s.overlay} onClick={() => setShowGoalForm(false)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={s.field}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {[
                  ...
                  <button
                    key={ico}
                    onClick={() => setGoalForm((f) => ({ ...f, icon: ico }))}
                    style={{
                      ...s.iconBtn,
                      ...(goalForm.icon === ico ? s.iconBtnActive : {}),
                    }}
                  >
                    {ico}
                  </button>
            ...
            <div style={s.field}>
              <label style={s.label}>Målbeløb (DKK)</label>
              <input
                type="number"
                placeholder="0"
            ...
            <div style={s.field}>
              <label style={s.label}>Allerede opsparet (DKK)</label>
              <input
                type="number"
                placeholder="0"
            ...
            <div style={s.modalBtns}>
              ...
              <button onClick={addGoal} style={s.confirmBtn}>
                Opret mål
              </button>
            </div>
          </div>
