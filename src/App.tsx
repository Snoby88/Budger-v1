      ...
      <div style={s.content}>
        ...
        {activeTab === 'chat' && (
          <div style={{ ...s.card, display: 'flex', flexDirection: 'column', height: 600 }}>
            <h3 style={s.cardTitle}>💬 Chat med din økonomi</h3>
            <div style={{ flex: 1, overflowY: 'auto', marginBottom: 16 }}>
              {chatMessages.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', marginBottom: 12 }}>
                  <div style={{ maxWidth: '80%', padding: '10px 14px', borderRadius: 12, background: m.role === 'user' ? '#6B8CFF' : '#1a2235', color: '#e2e8f0', fontSize: 14, lineHeight: 1.5 }}>
                    {m.content}
                  </div>
                </div>
              ))}
