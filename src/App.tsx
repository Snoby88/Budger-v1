...
export default function App() {
  ...
  const runAiAnalysis = async () => {
    if (!apiKey && !true) return; // apiKey check removed - using server env var
    setAiLoading(true);
    setAiError('');
    setAiAnalysis(null);
...
const styles = {
  ...
  header: { display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid #1e2d45', background: '#0d1520', position: 'sticky' as const, top: 0, zIndex: 100, flexWrap: 'wrap' as const },
  logo: { display: 'flex', alignItems: 'center', gap: 8 },
  logoText: { fontSize: 18, color: '#D4A853', fontWeight: 700 },
  nav: { display: 'flex', gap: 4, flex: 1, overflowX: 'auto' as const, msOverflowStyle: 'none' },
  navBtn: { background: 'none', border: 'none', color: '#64748b', padding: '6px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap' as const },
  ...
  content: { padding: '16px 12px', maxWidth: 1100, margin: '0 auto' },
  ...
  chartsRow: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginBottom: 16 },
  card: { background: '#111e2e', border: '1px solid #1e2d45', borderRadius: 12, padding: '16px 14px', marginBottom: 12 },
  ...
  goalsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12, marginBottom: 16 },
  ...
  modal: { background: '#111e2e', border: '1px solid #2d3f5e', borderRadius: 16, padding: '20px 16px', width: '95%', maxWidth: 420, maxHeight: '90vh', overflowY: 'auto' as const },
